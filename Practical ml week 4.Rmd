...
subTraining <- na.omit(subTraining)
...
### Decision tree
```{r decisiontree_predict, echo=TRUE}
# Fit model
...
```{r decisiontreecm_first, echo=TRUE}
# Convert both 'predictDT' and 'subTesting$classe' to factors
predictDT <- as.factor(predictDT)
subTesting$classe <- as.factor(subTesting$classe)

# Ensure they have the same levels
levels(predictDT) <- levels(subTesting$classe)

# Perform the confusion matrix calculation
confusionMatrix(predictDT, subTesting$classe)

```
### Decision tree
```{r decisiontree_fit, echo=TRUE}
# Fit model
modFitDT <- rpart(classe ~ ., data=subTraining, method="class")
...
```{r decisiontreecm_second, echo=TRUE}
confusionMatrix(predictDT, subTesting$classe)
```
# Perform prediction
